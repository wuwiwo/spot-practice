...
function render() {
  ...
  [...scenes].sort((a, b) => b.updatedAt - a.updatedAt).forEach(sc => {
    ...
    ex.title = '导出场景'; ex.addEventListener('click', e => { e.stopPropagation(); doExport(sc); });
    ...
    ed.title = '编辑场景'; ed.addEventListener('click', e => { e.stopPropagation(); navigate('editor', { sceneId: sc.id }); });
...
function doExport(sc) {
  ...
  a.href = url; a.download = (sc.name || 'scene').replace(/[^a-zA-Z0-9\u4e00-\u9fa5_-]/g, '_') + '.json';
  a.click();
...
$('#import-json-input').addEventListener('change', async () => {
  const f = $('#import-json-input').files?.[0]; if (!f) return;
  try {
    const text = await f.text();
    const { data, errors } = parseImportData(text);
    if (!data) { toast(errors[0] || '格式错误', 1); return; }
    toast('请选择对应场景图片');
    const inp = document.createElement('input');
    inp.type = 'file'; inp.accept = 'image/jpeg,image/png,image/webp';
    inp.addEventListener('change', async () => {
      const imgF = inp.files?.[0];
      if (!imgF) { toast('已取消', 1); return; }
      try {
